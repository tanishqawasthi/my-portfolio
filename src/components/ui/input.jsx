...
const Input = React.forwardRef(({ className, type, as = "input", ...props }, ref) => {
  ...
  const Component = as; // Use 'input' or 'textarea' based on the 'as' prop
      ...
      <Component
        ...
        className={cn(
          `flex w-full border-none bg-gray-50 dark:bg-zinc-800 text-black dark:text-white shadow-input rounded-md px-3 py-2 text-sm file:border-0 file:bg-transparent 
           ...
           ${as === "textarea" ? "h-32" : "h-10"}`, // Adjust height based on component type
          className
        )}
        {...props}
      />
